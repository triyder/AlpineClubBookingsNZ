...
| Membership lifecycle | Admin | Configure seasonal membership types, assign member seasonal types with guarded preview/reason/audit, roll assignments forward, and preserve role-based authorization | High | Schema, migration/backfill, admin route, service, and navigation tests | Targeted membership type seed/API/sidebar/member-assignment suites; no booking enforcement claims |
