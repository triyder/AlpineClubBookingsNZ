...
| Membership lifecycle | Admin/member | Configure seasonal membership types, assign member seasonal types with guarded preview/reason/audit, roll assignments forward, enforce booking rate/block policy, and surface effective subscription status without losing raw history | High | Schema, migration/backfill, admin route, service, pricing, booking gate, and navigation tests | Targeted membership type seed/API/sidebar/member-assignment suites plus membership-type policy, booking quote/create/modify, group join, and subscription-status tests |
| Committee administration | Admin/member/public | Configure committee master roles, link members to roles, edit hidden-by-default assignment presentation flags, and preserve legacy public committee records | High | Schema, migration/backfill, admin route, member detail, seed, audit, and public-source preservation tests | Targeted committee role/assignment API tests, seed defaults, member detail integration, and public committee API regression tests |
