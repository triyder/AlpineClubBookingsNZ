...
| Admin member management | Admin | Member detail grouped layout: pinned at-a-glance summary strip, default-collapsed groups with header previews, per-browser expand/collapse persistence, and transient `#account-credit` deep-link open | Medium | Component (jsdom) and helper unit tests | Targeted `admin-member-detail-groups` page suite (default collapse, toggle persistence, localStorage hydration, deep link) and `admin-member-detail-helpers` preview-formatter tests; Playwright browser spec tracked for the redesign's final stage |
