...
- **`ClubTheme` sub-AA gold theme bump is conditional** — it only rewrites a
  ...
  left as-is.
- **The generic sage-to-teal theme correction is fully guarded** — it updates
  only the incomplete `default` theme whose complete stored palette, fonts,
  logo, and raw CSS still exactly match the legacy generic defaults after the
  earlier gold bump. Completed, partially customised, and Tokoroa themes are
  left unchanged.
