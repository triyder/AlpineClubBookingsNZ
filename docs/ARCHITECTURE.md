...
step with the tree. The text-based assertions there strip comments with
TypeScript's own scanner before counting, because counting raw text conflates a
call site with prose *about* a call site (both `view-only-action.tsx`'s JSDoc
and `public-booking-requests-section.tsx`'s JSX commentary quote
`describeReason={false}` while explaining it, and each was miscounted as an
opt-out once). The #2168 checks go further and run over the parsed AST, where an
attribute is a node and prose is trivia, so that class of miscount cannot occur
at all. If you add or convert a gated control, that test fails and the numbers
here, in `AGENTS.md`, in `docs/STYLE_GUIDE.md`, in `CHANGELOG.md` and in the
`ViewOnlyActionButton` JSDoc all need updating together.
