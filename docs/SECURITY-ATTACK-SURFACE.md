...
and the route-count and guard-gap notes were refreshed on 2026-06-21 during the
issue #812 route-boundary review follow-up. The API count is from:

```bash
rg --files 'src/app/api' -g 'route.ts' | wc -l
```

Current count: 281 `src/app/api/**/route.ts` files.
...
| `/api/contact` | Public contact form. | Anonymous website users. | Name, email, message, optional published committee assignment recipient key. | SMTP/SES through `sendEmail()`. | Zod validation, CRLF checks, HTML escaping, `rateLimiters.contact` at 10/hour/IP. Committee recipient keys resolve server-side only when the assignment is active, published, contactable, and linked to an active member. | Email delivery logs through email layer; committee-routed messages store an opaque committee-contact marker instead of the private member recipient address; no audit log. | Spam and mailbox flooding are bounded but not CAPTCHA-backed. Invalid or non-contactable recipient keys safely fall back to the configured club contact address. |
...
- Public committee reads are capped to 50 active, published assignment records;
  ...
  assignments, member phone is returned only when show-phone is enabled, and
