...
| 💬 **In-app help everywhere** — a chat-style help widget on every surface (member, admin, finance, and the public site) that answers page-specific questions from a curated corpus and shows a full page guide → [architecture](docs/ARCHITECTURE.md) | 🐛 **One-click issue reporting** — members and staff capture the current view and report a problem from any page straight into the admin triage queue |
