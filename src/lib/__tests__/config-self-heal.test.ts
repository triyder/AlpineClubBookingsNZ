...
describe("runConfigSelfHeal — cold un-backfilled DB", () => {
  it("populates the identity row from the effective config on first run", async () => {
    ...
    const summary = await runConfigSelfHeal({ db, log: silentLog, provenance: "primary", steps: [clubIdentitySelfHealStep, clubFacebookUrlSelfHealStep] });
  ...
  it("is a no-op on the second run (idempotent)", async () => {
    ...
    await runConfigSelfHeal({ db, log: silentLog, provenance: "primary", steps: [clubIdentitySelfHealStep, clubFacebookUrlSelfHealStep] });
    ...
    const second = await runConfigSelfHeal({ db, log: silentLog, provenance: "primary", steps: [clubIdentitySelfHealStep, clubFacebookUrlSelfHealStep] });
...
describe("registry", () => {
  it("registers exactly the three known steps (identity first, facebookUrl + lodge-capacity alongside)", () => {
    // Length-pinned so a future step must consciously extend this assertion.
    expect(SELF_HEAL_STEPS).toHaveLength(3);
