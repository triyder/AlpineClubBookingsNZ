...
export default function FeeConfigurationPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle>Entrance fees</CardTitle>{data?.canEdit && !entranceEditing && <Button variant="outline" size="sm" aria-label="Edit entrance fees" onClick={() => setEntranceEditing(true)}>Edit</Button>}</CardHeader><CardContent className="space-y-5">
      {entranceEditing && <>
        ...
        <div className="flex gap-2"><Button disabled={saving} onClick={saveEntranceFee}>{editingEntranceFeeId ? "Update entrance fee" : "Add entrance fee"}</Button>{editingEntranceFeeId && <Button variant="outline" onClick={resetEntranceForm}>Cancel edit</Button>}<Button variant="ghost" disabled={saving} onClick={cancelEntranceEditing}>Close section</Button></div>
      </>}
