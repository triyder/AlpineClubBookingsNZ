...
export default function FeeConfigurationPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Annual membership fees</CardTitle></CardHeader><CardContent className="space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div><Label htmlFor="membership-type">Membership type</Label><Select value={membershipTypeId} onValueChange={setMembershipTypeId} disabled={!!editingMembershipFeeId || !data?.canEdit}><SelectTrigger id="membership-type"><SelectValue /></SelectTrigger><SelectContent>{data?.membershipTypes.map((type) => <SelectItem key={type.id} value={type.id}>{type.name}</SelectItem>)}</SelectContent></Select></div>
        <div><Label htmlFor="membership-amount">Annual amount (NZD)</Label><Input id="membership-amount" inputMode="decimal" value={membershipAmount} onChange={(event) => setMembershipAmount(event.target.value)} placeholder="150.00" disabled={billingBasis === "NO_INVOICE" || !data?.canEdit} /></div>
        <div><Label htmlFor="billing-basis">Billing basis</Label><Select value={billingBasis} onValueChange={(value) => { setBillingBasis(value); if (value === "NO_INVOICE") setMembershipAmount("0"); }} disabled={!data?.canEdit}><SelectTrigger id="billing-basis"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="PER_MEMBER">Per member</SelectItem><SelectItem value="PER_FAMILY">Per family</SelectItem><SelectItem value="NO_INVOICE">No invoice</SelectItem></SelectContent></Select></div>
        <div><Label htmlFor="proration-rule">Proration</Label><Select value={prorationRule} onValueChange={setProrationRule} disabled={!data?.canEdit}><SelectTrigger id="proration-rule"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="NONE">Full annual fee</SelectItem><SelectItem value="REMAINING_MONTHS_INCLUSIVE">Remaining months, including decision month</SelectItem></SelectContent></Select></div>
    ...
    <Card><CardHeader><CardTitle>Entrance fees</CardTitle></CardHeader><CardContent className="space-y-5">
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <div><Label htmlFor="entrance-amount">Amount (NZD)</Label><Input id="entrance-amount" inputMode="decimal" value={entranceAmount} onChange={(event) => setEntranceAmount(event.target.value)} placeholder="75.00" disabled={!data?.canEdit} /></div>
    ...
    <Card><CardHeader><CardTitle>Family billing members</CardTitle></CardHeader><CardContent className="space-y-3">
      ...
      {data?.familyGroups.map((group) => { const selectId = `family-billing-${group.id}`; return <div key={group.id} className="grid items-center gap-2 rounded-md border p-3 md:grid-cols-[1fr_2fr]"><div><div className="font-medium">{group.name || "Unnamed family"}</div>{group.billingException && <span className="text-sm text-amber-700">Billing exception</span>}</div><div><Label htmlFor={selectId}>Billing member</Label><Select value={group.billingMemberId ?? "none"} onValueChange={(value) => mutate({ action: "SET_FAMILY_BILLING_MEMBER", familyGroupId: group.id, billingMemberId: value === "none" ? null : value })} disabled={saving || !data.canEdit}><SelectTrigger id={selectId}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No billing member</SelectItem>{group.members.map((member) => <SelectItem key={member.id} value={member.id} disabled={!member.active}>{memberName(member)} · {member.email}{member.active ? "" : " (inactive)"}</SelectItem>)}</SelectContent></Select></div></div>; })}
    </CardContent></Card>
